...
function Login() {
  ...
  const loginCheck = async (data) => {
    ...
    await axios
      .post(loginApi, checkData, config)
      .then((response) => {
        // 登入成功後將表頭的token存起來
        ...
        if (newCheckEmail[0] == checkOneEmail) {
          ...
          axios
            ...
            .catch((error) => {
              toast.error(
                `${error.response.data.message}，請再次確認帳號密碼`,
                options
              );
            });
        } else {
            ...
            <form className='formControls'>
              ...
              >
                ...
                <input
                  ...
                  id='email'
                  name='email'
                  placeholder={`您好，尊敬的${userCheckName}會員，您目前為登入狀態`}
                  disabled
                />
              </div>
              <input
                ...
                onClick={signOut}
              />
              <Link className='formControls_btnLink' to='/todoList'>
                返回待辦頁面
              </Link>
            </form>
          )}
